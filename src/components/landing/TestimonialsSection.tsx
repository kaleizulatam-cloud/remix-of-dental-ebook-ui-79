...
const TestimonialsSection = () => {
  return <section className="py-24 bg-[#050B14] relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        loop: true
          ...
          <CarouselContent className="-ml-4">
            {testimonials.map(item => <CarouselItem key={item.id} className="pl-4 md:basis-1/2 lg:basis-1/3">
                <div className="bg-[#0B1221] border border-border/60 p-8 rounded-2xl shadow-xl flex flex-col justify-between h-full min-h-[320px] transition-all duration-300 hover:border-primary/20">
                  <div className="shadow-none">
                    {/* Stars */}
                    <div className="flex gap-1 text-primary mb-6">
                      {[...Array(5)].map((_, i) => <Star key={i} size={20} fill="currentColor" className="drop-shadow-[0_0_8px_rgba(34,211,238,0.4)]" />)}
                    </div>
                    {/* Testimonial Text */}
                    <p className="text-foreground/90 text-[15px] leading-relaxed mb-8 font-light font-sans px-0 py-[5px] mx-0">
                      "{item.text}"
                    </p>
                  </div>
          ...
          <div className="flex justify-center items-center gap-4 mt-12 mb-10">
            <CarouselPrevious className="static translate-y-0 bg-transparent border-border hover:bg-secondary hover:border-primary/50 text-muted-foreground hover:text-primary" />
            <CarouselNext className="static translate-y-0 bg-transparent border-border hover:bg-secondary hover:border-primary/50 text-muted-foreground hover:text-primary" />
          </div>
        </Carousel>
